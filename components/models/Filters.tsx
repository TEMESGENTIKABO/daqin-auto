...
export default function Filters({
  ...
  const categories = [
    { id: "all", name: "All", icon: <Grid className="w-4 h-4" /> },
      ...
      <div className="fixed inset-0 z-50 lg:hidden">
        ...
        >
          ...
          <div className="px-3 flex overflow-x-auto border-b border-gray-100 no-scrollbar">
            {[
              { id: "brands" as FilterTab, label: "Brands", icon: <Hash className="w-3.5 h-3.5" /> },
              { id: "categories" as FilterTab, label: "Type", icon: <Grid className="w-3.5 h-3.5" /> },
              { id: "price" as FilterTab, label: "Price", icon: <DollarSign className="w-3.5 h-3.5" /> },
              { id: "sort" as FilterTab, label: "Sort", icon: <Sparkles className="w-3.5 h-3.5" /> },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveFilterTab(tab.id)}
                className={`flex-shrink-0 px-3 py-2.5 flex items-center gap-2 transition-colors relative ${activeFilterTab === tab.id 
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                    {filteredBrands.map((brand) => (
                      <button
                        ...
                        className={`p-2 rounded-md flex items-center justify-between ${selectedBrands.includes(brand)
                            ? "bg-gold-primary/10 border border-gold-primary/30"
                            : "bg-gray-50 border border-gray-200"
                          }`}
                      >
                        <span className={`text-xs font-medium ${selectedBrands.includes(brand) ? "text-gold-primary" : "text-gray-700"}`}>
                          {brand}
                        </span>
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedBrands.includes(brand) 
    ...
    <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {categories.map((category) => (
            <button
              ...
              className={`p-2 text-xs rounded-md flex flex-col items-center gap-1 transition-colors ${selectedCategory === category.id
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          >
            {selectedBrands.length === brandsList.length ? "Clear" : "All"}
          </button>
        </div>
        ...
        <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
          {visibleBrands.map((brand) => (
            <button
              key={brand}
              onClick={() => onToggleBrand(brand)}
              className={`w-full p-2 rounded-md flex items-center justify-between text-sm ${selectedBrands.includes(brand)
                  ? "bg-gold-primary/10 text-gold-primary"
                  : "hover:bg-gray-100 text-gray-700"
                }`}
            >
              <span className="font-medium">{brand}</span>
              <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedBrands.includes(brand) 
                  ? "bg-gold-primary border-gold-primary" 
                  : "border-gray-300"
                }`}>
                {selectedBrands.includes(brand) && (
          ...
          {brandsList.length > 6 && (
            ...
            >
              ...
              {brandsExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="space-y-1.5">
          {pricePresets.map((preset) => (
            <button
              ...
              className={`w-full p-2 text-xs rounded-md text-left ${pricePreset === preset.id
...
function CategoriesFilter({ categories, selectedCategory, onSelectCategory, isMobile }: {
      ...
      <div className="grid grid-cols-2 gap-2">
        {categories.map((category) => (
          <button
            key={category.id}
            onClick={() => onSelectCategory(category.id)}
            className={`p-2.5 rounded-md text-xs flex flex-col items-center gap-1.5 ${selectedCategory === category.id
                ? "bg-gold-primary text-white"
                : "bg-gray-50 border border-gray-200"
              }`}
          >
...
function PriceFilter({ pricePreset, pricePresets, onPricePresetChange, isMobile }: {
  pricePreset: PricePreset;
  pricePresets: Array<{ id: PricePreset; label: string; range: [number, number] }>;
  onPricePresetChange: (preset: PricePreset) => void;
  isMobile: boolean;
      ...
      <div className="grid grid-cols-2 gap-2">
        {pricePresets.map((preset) => (
          <button
            key={preset.id}
            onClick={() => onPricePresetChange(preset.id)}
            className={`p-2.5 rounded-md text-xs ${pricePreset === preset.id
                ? "bg-gold-primary text-white font-medium"
                : "bg-gray-50 border border-gray-200"
              }`}
          >
...
function SortFilter({ sortBy, onSortChange, isMobile }: {
  sortBy: SortOption;
  onSortChange: (sort: SortOption) => void;
  isMobile: boolean;
}) {
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        {[
          { id: "default" as SortOption, label: "Recommended", icon: <Sparkles className="w-4 h-4" /> },
          ...
          <button
            ...
            className={`w-full p-2.5 rounded-md flex items-center justify-between text-sm ${sortBy === option.id
                ? "bg-gold-primary/10 text-gold-primary"
                : "bg-gray-50 border border-gray-200"
              }`}
          >
